...
export class Buffer {
  ...
  export(n) {
    ...
    const exported = this.buffer.slice(this.used, this.used + remain);
...
export class Multiple {
  ...
  async export(n) {
    for (;;) {
      if (this.reader !== null) {
        const exported = await this.reader.reader.export(n);
...
export async function readN(reader, n) {
  ...
  while (readed < n) {
    const exported = await reader.export(n - readed);
...
export class Limited {
   ...
   */
  buffered() {
    const buf = this.reader.buffered();
  ...
  async export(n) {
    ...
    const toRead = n > this.remain ? this.remain : n;
    const exported = await this.reader.export(toRead);
...
export async function readUntil(indexOfReader, byteData) {
  const pos = await indexOfReader.indexOf(byteData);
  const buffered = await indexOfReader.buffered();
